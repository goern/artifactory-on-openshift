...
TIP: A fully working example of an Jenkins container image with Artifactory plugin and some other additional plugins is available online at https://github.com/goern/jenkins-acme
...
This Jenkins pipeline snippet will define some Artifactory related variables and configure Maven related items of the Artifactory plugin. After the source code has been checked out, a Maven build using the Artifactory configuration is started. Any build artifacts will resist within the Maven container's storage, they are neither packaged into a container image, nor are they uploaded to Artifactory. Processing the build artifacts is the responsibility of follow on pipeline stages.
