...
The following chapters will guide you through the steps to prepare the deployment of JFrog Artifactory container images, create an application template for Artifactory on OpenShift and deploy all the components to OpenShift by using this template.

===== JFrog Artifactory container image

JFrog does provide Artifactory container images, these will be use with the OpenShift template of this {RefImpl}.
...
JFrog Artifactory container images require root permission during startup, so it is required that you create an OpenShift service account and grant the privileges to run as any user id:
...
WARNING: To perform the second command, you need to be at least an OpenShift admin or cluster-admin.
...
The file `artifactory.lic` is required only if using Pro version. A Pro version is required to use the REST API or docker repositories.
...
These steps are implemented and provided for your convenience as an <<https://gitlab.com/goern/artifactory-on-openshift/blob/master/playbooks/create-default-repositories.yaml,Ansible playbook>>.
...
an <<https://gitlab.com/goern/artifactory-on-openshift/blob/master/playbooks/create-default-repositories.yaml,Ansible playbook is provided>> which will create the following repositories:
...
TIP: You can find an example S2I NGINX repository at https://gitlab.com/goern/s2i-nginx/tree/acme-it-dev
...
After you have created the S2I NGINX repository you need to add the Artifactory configuration snippet. Artifactory provides a restful API to do this, and the configuration snippet could be obtained very simple via `curl -gO http://artifactory.example.com/api/system/configuration/reverseProxy/nginx`. Move this file into `etc/conf.d/artifactory.conf` of the S2I NGINX repository and add and commit it too.
...
TIP: Again, you can find an example repository at https://gitlab.com/goern/artifactory-on-openshift/tree/master/container-image/reverse-proxy-nginx
...
All these objects could be created from an {OCP} template, and this could be automated with an Ansible playbook. Such a playbook is provided as https://gitlab.com/goern/artifactory-on-openshift/blob/master/playbooks/deploy-service.yaml[`playbooks/deploy-service.yaml`]. All the tasks have been tagged, so you can easily select either to deploy Artifactory itself, or just the NGINX-based reverse proxy.
...
PLAY [Deploy Artifactory, see https://gitlab.com/goern/artifactory-on-openshift] ***
...
$ curl -gO http://artifactory.example.com/api/system/configuration/reverseProxy/nginx
...
TIP: A fully working example of an Jenkins container image with Artifactory plugin and additional plugins is available online at https://gitlab.com/goern/jenkins-acme
...
spec:
  ...
  source:
    ...
    git:
      ref: acme-demo <3>
      uri: https://gitlab.com/goern/jboss-eap-quickstarts.git <1>
    type: Git
  output:
    to:
      kind: ImageStreamTag
...
node('maven') { // this will use maven jenkins slave containers...
  ...
  stage('SCM: checkout') {
    git url: 'https://gitlab.com/goern/jboss-eap-quickstarts.git', branch: 'acme-demo'
  }
